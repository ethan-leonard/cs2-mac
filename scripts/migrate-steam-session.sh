...
usage() {
  cat <<'EOF'
Usage:
  migrate-steam-session.sh [--dry-run|--apply|--rollback]
...
What it does:
  - Moves Steam session state out of the CrossOver Steam bottle and into the new prefix Steam root.
  - Handles config/config.vdf, config/loginusers.vdf, and userdata/.
...
Examples:
  migrate-steam-session.sh --dry-run
  migrate-steam-session.sh --apply
  migrate-steam-session.sh --rollback
EOF
}
...
run_migration() {
  ...
  if [[ "$source_has_any" -eq 0 && "$target_has_any" -eq 0 ]]; then
    die "no Steam session artifacts were found in either source or target Steam roots"
  fi

  if [[ "$mode" == "dry-run" ]]; then
    log "dry-run complete; rerun with --apply to perform the moves"
  else
    log "session migration complete; rollback journal: $journal"
  fi
}
